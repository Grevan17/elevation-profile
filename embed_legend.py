...
class DraggableHeader(QLabel):
    """Custom QLabel to handle parent dock widget dragging."""
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            if self.parent_dock and not sip.isdeleted(self.parent_dock):
                ...
                self.drag_start_position = global_pos - self.parent_dock.frameGeometry().topLeft()
    ...
    def mouseMoveEvent(self, event):
        if event.buttons() & Qt.LeftButton and self.drag_start_position:
            if self.parent_dock and not sip.isdeleted(self.parent_dock):
                global_pos = event.globalPosition().toPoint() if hasattr(event, 'globalPosition') else event.globalPos()
                self.parent_dock.move(global_pos - self.drag_start_position)
...
class EmbedLegendPlugin:
    ...
    def disconnect_signals(self):
        try: 
            self.iface.layerTreeView().selectionModel().selectionChanged.disconnect(self.update_legend)
        except Exception: 
            pass
        try: 
            self.iface.mapCanvas().mapCanvasRefreshed.disconnect(self.update_legend)
        except Exception: 
            pass
    ...
    def on_item_clicked(self, item):
        ...
        try:
            ...
            if hasattr(renderer, 'legendSymbolItemChecked'):
                current_state = renderer.legendSymbolItemChecked(rule_key)
                new_state = not current_state
                if hasattr(renderer, 'checkLegendSymbolItem'):
                    renderer.checkLegendSymbolItem(rule_key, new_state)
            ...
            if tree_layer:
                ...
                for node in nodes:
                    ...
                    if str(node_key) == str(rule_key):
                        node_state = node.data(Qt.CheckStateRole)
                        n_state = Qt.Unchecked if str(node_state) not in ("0", "Unchecked", "False", "None") else Qt.Checked
                        node.setData(n_state, Qt.CheckStateRole)
                        break
            ...
            print(f"Embed Legend Click Error: {e}")
    ...
    def update_legend(self):
        ...
        try:
            from qgis.core import QgsSymbolLayerUtils, QgsProject, QgsVectorLayer, QgsRenderContext, QgsFeatureRequest
                ...
                if self.show_count or self.show_percent:
                    try:
                        ...
                        for feat in layer.getFeatures(req):
                            keys = renderer.legendKeysForFeature(feat, ctx)
                            for k in keys:
                                counts[k] = counts.get(k, 0) + 1
                                total_f += 1
                        renderer.stopRender(ctx)
                    except Exception as e:
                        print(f"Embed Legend Counting Error: {e}")

                for i, r_item in enumerate(r_items):
                    rule_key = r_item.ruleKey()
                    txt = r_item.label()
                    
                    if hasattr(renderer, 'legendSymbolItemChecked'):
                        is_checked = renderer.legendSymbolItemChecked(rule_key)
                    else:
                        is_checked = True 
                    ...
                    cnt = counts.get(rule_key, 0)
                    if self.show_count:
                        txt += f" [{cnt}]"
                        
                    if self.show_percent and total_f > 0:
                        pct = (cnt / total_f) * 100
                        txt += f" ({pct:.1f}%)"
                    ...
                    item = QListWidgetItem(icon, txt)
                    item.setData(Qt.UserRole, str(layer.id()))
                    item.setData(Qt.UserRole + 1, str(rule_key)) 
            ...
            print(f"Embed Legend Update Error: {e}")
        ...
        try:
            ...
            with open(mif_path, 'w', encoding='latin-1', errors='replace') as f_mif, \
                ...
                for field in fields:
                    ...
                    if not col_name: 
                        col_name = f"Col_{fields.indexOf(field)}"
                ...
                for i, feat in enumerate(layer.getFeatures()):
                    ...
                    progress.setValue(i)
                    try:
                        attrs = [str(a) if a is not None else "" for a in feat.attributes()]
                    ...
                    if QgsWkbTypes.geometryType(wkb_type) == QgsWkbTypes.PointGeometry:
                        ...
                        for poly in polys:
                            ...
                            for j in range(1, len(poly)): 
                                inner_coords = " ".join([f"{p.x()},{p.y()},0" for p in poly[j]])
